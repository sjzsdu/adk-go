...
func TestNewModel(t *testing.T) {
	...
	m, err := NewModel(ctx, ModelQWenMax, cfg)
	if err != nil {
	...
	_, err = NewModel(ctx, ModelQWenMax, cfg)
	if err == nil {
...
func TestGetSupportedModels(t *testing.T) {
	...
	if !found {
		t.Errorf("GetSupportedModels() should contain %s, but it doesn't", ModelQWenMax)
	}
}
...
func TestModel_Generate(t *testing.T) {
	...
	ctx := context.Background()
	cfg := Config{APIKey: "test-key"}
	model, err := NewModel(ctx, ModelQWenMax, cfg)
	if err != nil {
